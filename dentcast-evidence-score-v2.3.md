# DentCast Evidence Score (DES) — v2.3
...
```json
{"des_version":"2.3","error":"INSUFFICIENT_TEXT"}
{"des_version":"2.3","error":"DOI_TEXT_MISMATCH"}
```
...
1. **`low`** — a list of the excluded full-text studies is provided, each with
   its reason.
...
This is DES v2.3. If scoring criteria change in the future, the version number must change and old scores must not be silently compared with new ones. Store the version with every published score.
