...
import { el, faNum, tehranDay, sectionIcon } from './util.js?v=157';
import { certificatesBody } from './certificates.js?v=157';
import { api, ApiError, currentUser } from './api.js?v=157';
import { remindersBlock } from './reminders.js?v=157';
import { telegramLoginEnabled, telegramCallbackUrl, telegramBotUsername } from './config.js?v=157';
import { baleEnabled, baleDeepLink } from './config.js?v=157';
import { leagueEntryButton } from './league.js?v=157';
import { achievementsBody, discountBody, maybeCelebrate } from './achievements.js?v=157';
import { subscriptionCta } from './premium-cta.js?v=157';
import { copyToClipboard, confirmStrip, toast } from './hl-view.js?v=157';
...
export async function renderProfile(root, { me: preMe } = {}) {
  ...
  // Same contract as the account menu's «خروج» (header.js): say something while
  // it runs, and never navigate away on a failure — the session cookie is
  // httpOnly, so a logout that did not reach the server has cleared nothing and
  // the homepage would simply show the reader signed in again.
  ...
  root.replaceChildren(
    ...
    section('یادآوری‌ها', remindersBlock(me), 'reminders'), // #reminders: the پریمیوم tab's پیامک card lands here
