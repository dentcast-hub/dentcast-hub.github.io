...
Rules (see SEO audit / task brief):
  ...
  - About page (about.html): 3-line block like every other fa page
  - English mirrors (`{type}/en/{file}.html`, incl. metanotes/en/about.html):
    NO hreflang block — the en mirrors are AI-generated translations
    without editorial review, so they are noindexed and out of the
...
def desired_block(rel_path: str, canonical_path: str, indent: str) -> list:
    ...
    # About page (fa) — 3-line block like every other fa page. The en about
    # page is handled by the en rule below (no hreflang), same as every
    # other en mirror.
    if p == "about.html":
        return make_block(indent, [
    ...
    # English page of ANY type: {type}/en/{file}.html — NO hreflang block.
    # The en mirrors are AI-generated translations without editorial review,
    # so they are noindexed (tools/noindex_en_mirrors.py) and removed from
    # the hreflang cluster and the sitemap: a noindexed page must not be
    # advertised as an alternate. The fa↔en toggle stays — it is a reader
    # feature, not a search feature. (See the 2026-08-11 SEO audit.)
    if p == "metanotes/en/about.html" or fa_counterpart_path(p) is not None:
        return []

    # NOTE: fa pages are deliberately NOT paired to their en mirrors any
    # more (the old disk-discovered `en` alternate line is gone for the
    # same noindex reason above) — every fa typed page falls through to
    # the default 3-line block.
