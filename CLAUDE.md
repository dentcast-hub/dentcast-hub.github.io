...
- **Dead-man's switch** (`plus-api/src/services/failsafe.ts`, migration 0018). After a long silence from the founder, every signed-in reader is treated as premium, so the paid half of the site cannot be orphaned behind a door only one person could open. Any founder action is a heartbeat — a publish, any authenticated `/admin` request, or `POST /admin/failsafe/heartbeat` — and restarts the clock; the trigger is deliberately **not** "no new post for N months", because illness/travel/burnout produce the same silence as death. 60 days → warnings to the founder; 90 days → it arms. Arming sets **one flag**, never a bulk rewrite of `profiles.tier`: the entitlement is layered on at read time in `middleware/auth.ts`, so it covers users who sign up afterwards, reverses with one `UPDATE`, and leaves the column honest for the league prize and any future payment flow. `/me` therefore carries `tier` (effective), `base_tier` (as stored) and `failsafe_premium` — **anything asking what an account really is must read `base_tier`**. Two consequences that are easy to break: **ads keep running** when it is armed (`spot.js` targets on `base_tier`, because ad revenue pays for the server that serves the features the switch just unlocked), and returning does **not** auto-disarm (that is an explicit `POST /admin/failsafe/disarm`). Set `FAILSAFE_FOUNDER_USER_ID` or the warnings have nowhere to go. Docs: `plus-api/README.md` → "The dead-man's switch".
