...
- **The paper cabinet is the one allowance the server cannot enforce**, and it is deliberate rather than forgotten: `dentcast_cabinet_search.html` is a static page that fetches a public JSON and does all its searching, filtering and sorting in the browser, with no API in its path. So the counter is device-local (`dc.cab.q.<Tehran-day>` in localStorage) and metered on **distinct committed queries**, not keystrokes. Three searches a day; then search AND sort lock together (over 2200 papers they are one tool) while the full list stays browsable — «دیدن فهرست کامل» clears the filters. It **fails open**: an unreachable `/me` leaves the page unmetered, because a subscriber locked out by a network blip is far worse than a visitor getting a fourth query. The homepage's `#card-library` no longer gates the tap at all — it used to hide 2200 papers behind a sheet, so the card advertised something nobody could look at.
