...
- `dentcast_cabinet_full_catalog.json` — root catalog of the paper cabinet (`papers` array; mirrors the Google Drive folder's topical subfolders). Updated by the paper branch (workflow step 4.10); the search UI is `dentcast_cabinet_search.html`. **Its kebab-style `tags` are a SEPARATE system from the brain `#hashtags` above — not governed by the hashtag reference library.** Three things the cabinet now carries that a publish must not undo: it is **kept out of search** (`robots.txt` disallows the catalog and its `.enriched.json` sibling, `_headers` adds `X-Robots-Tag: noindex`, the page carries its own `<meta robots noindex>`) — never "fix" that and never sitemap it; on the site the PAGE is open to everyone («کتابخانهٔ دنت‌کست», the second card of the archive tab's «موتور جستجوی دندانپزشکی» and the «کتابخانه» tool in the hamburger) and only the FILES are premium — `#card-library` no longer gates the tap, and the browser is served the link-free `plus/library-index.json`, so **re-run `python3 tools/build_library_index.py` after step 4.10** or a newly filed paper will not appear on the page at all; and the card + its gate sheet advertise a **hand-written paper count** («بیش از ۲۲۰۰ مقاله») that step 4.10 sweeps on both surfaces when the total crosses a round hundred.
...
- `plus/quota.json` — **the free tier's allowance on every premium feature**, and the reason none of them is a locked door any more. Same machinery as `badges.json`/`pathways.json`: versioned in the repo, loaded by `plus-api/src/quota.ts`, re-fetched live by `content-refresh.ts` — so **retuning a limit is a commit to the site, not an API deploy**, which is the whole point, because a quota number is a price and prices get re-answered by the funnel. Enforced by `plus-api/src/services/quota.ts`. Five rules the design rests on: **every counter is DERIVED, never written down** — review cards from `user_activity`, assistant cases from `assistant_rounds`, boards from `collections` — so the system needed **no migration at all**, has nothing to backfill, and cannot drift from the log it came from (same doctrine as the badges wall); **the boundary is always where the data is produced, never where it is drawn** — a hidden button is not a quota, so a route either refuses (402 `quota_exhausted`) or truncates its own payload before it leaves the process, and `plus/js/quota.js` only ever draws the count and the copy; **two kinds of wall, worded differently on purpose** — a spent allowance refills and says when (`resets_at`), while a preview boundary (`premium_required`: the locked pathways, the compass's blind-spot map) never does and must not pretend to; **a consumable check is taken under a lock** (`consumeQuota`'s `pg_advisory_xact_lock`, in the same transaction as the row it counts) because check-then-consume is a real bypass, not a theoretical one; and **the assistant charges the CASE, not the round** — identity is the server's own hash of the description (`caseRunHash`), so a fabricated history cannot walk past the limit and continuing an admitted case is always free. What is NOT metered: the three continuous perks (no ads, instant new-article notification, the score multiplier), which have nothing to sample. Tests: `plus-api/test/quota.test.ts`, plus the free-tier block in each feature's own suite.
- **The paper cabinet's boundary is the PAPER, not the search** (`plus/library-index.json`, `plus-api/src/library.ts`, `routes/library.ts`, quota key `library_papers`). Searching, filtering and sorting the whole 2200-paper catalogue is free for everyone, signed in or not; what premium buys is opening the file. This is the second design — the first metered *search* with a localStorage counter, which was the wrong boundary twice over: unenforceable (a counter on the reader's own machine) and selling the wrong thing (the ability to look, rather than the thing being looked for). Three things make the new one real: the page is served `plus/library-index.json` (built by `tools/build_library_index.py` — titles/tags/ids and **no `drive_*` field at all**, 721 KB instead of the 6 MB master catalogue it used to download whole), so a Drive URL is never in the payload; `GET /library/paper/:id` is the only producer of one and checks the subscription first; and a paper is charged **once per period however many times it is opened** (`count(distinct content_id)`), so re-reading is free and nobody is taught to hoard downloads. An unknown id and a known id with no file answer identically, so the route cannot be used to enumerate. **What this does NOT fix, and must never be described as fixing:** the master catalogue is a file in a *public* repo and git history keeps every version, so URLs already published stay findable, and a Drive file shared by link stays reachable to anyone holding one. The durable fix is Drive-side — unshare the folder and mint short-lived URLs from a service account — and it plugs in at `resolvePaper()`.
- `plus/badges.json` — the achievement catalog behind the profile's «افتخارات» section (two league medals + 23 badges). Same machinery as `pathways.json`: versioned in the repo, loaded by `plus-api/src/badges.ts` (reload-on-change, remote copy wins, last-good on error), re-fetched live by `content-refresh.ts`, so **retuning a threshold or rewriting a badge's Persian copy is a commit to the site, not an API deploy**. Served by `GET /achievements` (`plus-api/src/routes/achievements.ts`), rendered by `plus/js/achievements.js` into `profile.js` between «رکوردها» and «لیگ من». Four rules the code depends on: **every badge is DERIVED, never written down** (`plus-api/src/services/achievements.ts` computes all 23 metrics from data that already exists) — which is what makes the whole shelf retroactive, with no `earned_at` column that could disagree with the log it came from and no backfill on launch day; **one comparison, `value >= threshold`**, so anything naturally inverted (signup order in «پیشگام») is normalised to a 0/1 in the service rather than teaching the evaluator a second direction; **a level is a ring colour, never a word** — the wall never writes «طلا», only the medal row does and there it never stands alone («طلای کامپوزیت»), because two metal scales share the section (the league's seven dental materials and the badges' bronze/silver/gold) and the word is what would collide; and **any query touching league rank carries the validity filter** (group `>= min_valid_group_size` **and** `weekly_xp > 0`) because `league-finalize.ts` writes `final_rank` for every member of every group, including ones the UI itself calls non-competitive — without it, most accounts' first medal would be minted for doing nothing. `visibility` decides what an unearned badge looks like: `always` (grey, with its criterion), `mystery` (a «؟» tile that leaks nothing — the server strips the name, icon and copy), `earned_only` (absent entirely, for a condition nobody can meet any more). Tests: `plus-api/test/achievements.test.ts` + `achievements.dom.test.ts`.
