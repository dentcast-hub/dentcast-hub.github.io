...
- `dentcast_cabinet_full_catalog.json` — root catalog of the paper cabinet (`papers` array; mirrors the Google Drive folder's topical subfolders). Updated by the paper branch (workflow step 4.10); the search UI is `dentcast_cabinet_search.html`. **Its kebab-style `tags` are a SEPARATE system from the brain `#hashtags` above — not governed by the hashtag reference library.** Three things the cabinet now carries that a publish must not undo: it is **kept out of search** (`robots.txt` disallows the catalog and its `.enriched.json` sibling, `_headers` adds `X-Robots-Tag: noindex`, the page carries its own `<meta robots noindex>`) — never "fix" that and never sitemap it; on the site it is **premium** («کتابخانهٔ دنت‌کست», the second card of the archive tab's «موتور جستجوی دندانپزشکی» and the «کتابخانه» tool in the hamburger), gated on the tap by `#card-library` in `index.html`, so verify a new paper by opening the search page directly; and the card + its gate sheet advertise a **hand-written paper count** («بیش از ۲۲۰۰ مقاله») that step 4.10 sweeps on both surfaces when the total crosses a round hundred.
