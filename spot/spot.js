...
import { findProseRoot } from '/plus/js/config.js?v=126';
...
function report(kind, slotName, creativeId) {
  ...
  import('/plus/js/api.js?v=126')
    ...
    .catch(() => {
      ...
      import('/plus/js/api.js?v=126').then((m) => m.forgetBase()).catch(() => {});
    });
}
...
function pageType() {
  ...
  // Pillar hub (/pillar/) and the per-pillar pages (/pillar/<slug>/). Both are
  // generated by tools/build_pillar.py and both open with .pillar-intro, so one
...
function renderHome(cfg, creative) {
  ...
  // most one is visible.
  //
  // Mobile shell: the card sits ABOVE «دسته‌های محتوا» (founder placement,
  // 2026-08-12) — i.e. right after the featured episode, higher up the page
  // than it used to be. The anchor is the grid's own SECTION HEADING, not the
  // grid: inserting before the grid itself would drop the card between a
  ...
  if (wf) {
    ...
    const pulse = document.querySelector('#dcPulseCard');
    if (pulse) {
      pulse.parentNode.insertBefore(buildCard(creative, 'home'), pulse.nextSibling);
      placed = true;
    }
  }
...
// Pillar pages are SEO landing pages: most of their traffic arrives from a search
// engine and is therefore signed-out — the one audience the "join Plus" card is
// targeted at and actually converts on. They carried no slot at all until now,
// while still counting in the page-view denominator.
//
// The card sits directly after the intro prose, before the first subtopic card:
// the reader has just finished the framing paragraphs and is about to start
// scanning lists, so it is a real boundary rather than an interruption, and it is
// high enough on the page to be reached without a deep scroll.
function renderPillar(cfg, creative) {
  const intro = document.querySelector('.pillar-intro');
...
function viewerProbe() {
  return import('/plus/js/api.js?v=126')
