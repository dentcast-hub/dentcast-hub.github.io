...
**0.1 — Source of truth is `litecast.json`, not `dentcast-brain.json`.**
- Infer **all** LiteCast structure from `litecast.json`: required fields, schema shape, directory/URL pattern, and media requirements. Read it the way Phase A reads the brain — find the **most recent** LiteCast entry and use it as both the **schema** and **structural** template.
- The new LiteCast entry is added **only** to `litecast.json`, matching its format **exactly** (identical keys/nesting/ordering to the previous entry — no extra fields, no missing fields).
...
**0.4 — What still runs (keyed to `litecast.json` and the LiteCast directory, not the brain).** Everything that is **not** cross-linking still applies in LiteCast-appropriate form:
...
- **Step 5 / 5.5** — replaced by 0.1: the entry goes to `litecast.json` only. LiteCast lives **outside** the specialist pillar taxonomy, so do **not** assign a specialist pillar/subtopic and do **not** pull from the `PILLARS` taxonomy; fill a pillar-like field only if `litecast.json`'s own schema has one, per its own convention.
...
Enriches the **new content's own** "کاوش بیشتر" section with links pointing **out** to semantically related existing brain entries. Direction matters — don't confuse this with the neighbouring steps:
