...
**Episodes note:** the episodes landing page is `/episodes.html` (not `/episodes/index.html`, which is only a redirect). It is a **fully static** page (NOT client-rendered), so appending to `dentcast.json` in step 5 is **not enough** — the new episode must be added to `episodes.html` itself in **step 8** (builder #3). ⚠️ Its generator `tools/build_episodes.py` is currently **out of sync** with the hand-maintained `episodes.html` and would **regress** curated features if run blindly — see step 8 builder #3 for the exact procedure (surgical add vs. fixing the builder first). Do not assume `/episodes.html` picks up new episodes at runtime.
...
3. **Episodes landing-page (`episodes.html`) — MANDATORY whenever the publish touched `dentcast.json` (any episode publish).** `episodes.html` is a **fully static** page (episode list, featured-card skeleton, stats and Jalali dates baked in — **not** client-rendered, so a `dentcast.json` edit alone does NOT surface the new episode under `/episodes/`). The new episode (newest-first → **top** of `<ol id="episodeList">`) plus the episode-count stats (hero «اپیزود» count, «ساعت محتوا», the archive `<span class="count">`, and the «۲۰۵ اپیزود» strings in `<meta>`/OG/Twitter/JSON-LD) must be updated. `episodes/index.html` is just a redirect to `/episodes.html` — never hand-edit it.

   ⚠️ **CRITICAL — `tools/build_episodes.py` is currently OUT OF SYNC with the curated `episodes.html`; do NOT blindly run it.** The committed `episodes.html` has been hand-maintained ahead of the builder and carries features the builder does **not** emit (the «جدیدترین⇄قدیمی‌ترین» sort toggle `archive-sort`, «Next 10» JS pagination, the `dc-jump` button, the site-wide «پرامپتولوژیست» search filter, and current asset versions like `dc-nav.css?v=12` / `global-search.css?v=2`). Running the builder as-is **regresses** all of these. Until the builder is brought back in sync with the curated page, do **one** of:
   - **(preferred, low-risk)** add the new episode **surgically** to the curated `episodes.html` — clone an existing `<li>` in `<ol id="episodeList">`, swap in the new episode's number/title/duration/Jalali-date/caption/first-3-hashtags/`page_url`, place it at the **top**, and bump the count stats listed above. Verify the **only** diff vs the prior `episodes.html` is the new `<li>` + the count bumps (no feature lost). This is how episode 158 was added.
   - **(proper fix)** first reconcile `tools/build_episodes.py` so its output equals the curated page (port the toggle/pagination/`dc-jump`/promptologist/asset-versions into its locked constants), confirm a fresh run diffs against the curated page by **only** the new episode + count strings, then run it.

   Either way: it never touches individual `/episodes/episode-*.html` pages, so hand-added cross-links (steps 4.8/4.9) are safe. Skip this whole step only for non-episode types that don't write to `dentcast.json`.
...
- Builder runs: each command + full stdout/stderr (`python tools/update-homepage-counters.py`, `python tools/build_pillar.py all`, and `python tools/stamp-version.py` LAST); confirmation that the new content appears in the regenerated pillar page when a structured pillar was assigned. **For episode publishes also report how `episodes.html` was updated (per step 8 #3 — surgical add to the curated page, or a reconciled-builder run — NOT a blind `build_episodes.py` run, which regresses curated features) and confirm the new episode now appears in `episodes.html` with no feature regression.**
