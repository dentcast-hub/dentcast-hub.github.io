# DentCast Evidence Score (DES) — v2.0
...
```json
{"des_version":"2.0","error":"INSUFFICIENT_TEXT"}
{"des_version":"2.0","error":"DOI_TEXT_MISMATCH"}
```
...
This is DES v2.0. If scoring criteria change in the future, the version number must change and old scores must not be silently compared with new ones. Store the version with every published score.
